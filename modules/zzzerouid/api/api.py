# BASE_URL
GS_BASE = "https://api-takumi.mihoyo.com"
GS_BASE_OS = "https://api-os-takumi.mihoyo.com"
RECORD_BASE = "https://api-takumi-record.mihoyo.com"

ZZZ_API = f"{RECORD_BASE}/event/game_record_zzz/api/zzz"
ZZZ_OS_API = "https://sg-act-nap-api.hoyolab.com/event/game_record_zzz/api/zzz"

ZZZ_INDEX_API = "/index"
ZZZ_NOTE_API = "/note"
ZZZ_BUDDY_INFO_API = "/buddy/info"
ZZZ_AVATAR_BASIC_API = "/avatar/basic"
ZZZ_AVATAR_INFO_API = "/avatar/info"
ZZZ_CHALLENGE_API = "/challenge"
ZZZ_ABYSS_API = "/abyss_abstract"
ZZZ_HADAL_API = "/hadal_info_v2"
ZZZ_NOTE_WIDGET_API = "/widget"
ZZZ_MONTH_INFO = "/month_info"
ZZZ_MEM = "/mem_detail"
ZZZ_VOID_BATTLE_API = "/void_front_battle_detail"

ZZZ_BIND_API = f"{GS_BASE}/binding/api"
ZZZ_BIND_OS_API = f"{GS_BASE_OS}/binding/api"
ZZZ_GAME_INFO_API = "/getUserGameRolesByCookie?game_biz=nap_cn"

ZZZ_RES = "https://act-webstatic.mihoyo.com/game_record/zzz"
NEW_ZZZ_RES = "https://act-webstatic.mihoyo.com/game_record/nap"
V2_ZZZ_RES = "https://act-webstatic.mihoyo.com/game_record/zzzv2"

ZZZ_SQUARE_AVATAR = f"{ZZZ_RES}/role_square_avatar"
ZZZ_SQUARE_BANGBOO = f"{ZZZ_RES}/bangboo_rectangle_avatar"
V2_ZZZ_SQUARE_AVATAR = f"{V2_ZZZ_RES}/role_square_avatar"
V2_ZZZ_SQUARE_BANGBOO = f"{V2_ZZZ_RES}/bangboo_rectangle_avatar"

PUBLIC_API = "https://public-operation-nap.mihoyo.com"
PUBLIC_GACHA_LOG_API = f"{PUBLIC_API}/common/gacha_record/api"
ZZZ_GET_GACHA_LOG_API = f"{PUBLIC_GACHA_LOG_API}/getGachaLog"

ANN_API = "https://announcement-api.mihoyo.com/common/nap_cn/announcement/api"

GET_AUTHKEY_URL = "https://api-takumi.mihoyo.com/binding/api/genAuthKey"

ENKA_API = "https://enka.network/api/zzz/uid/{}"
MINIGG_API = "https://profile.microgg.cn/api/zzz/uid/{}"
