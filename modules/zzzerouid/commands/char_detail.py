...
from ..utils.char_offset import char_offset
...
from ..utils.name_convert import char_id_to_full_name, char_name_to_char_id
from ..utils.resource import PLAYER_PATH, ROLE_PATH, TEXTURE2D_PATH
from ..utils.uid import get_uid

__all__ = ["draw_card", "refresh_card"]

TEXT_PATH = TEXTURE2D_PATH / "zzzerouid_char_detail"
...
async def refresh_card(msg: Bot.MessageSession, name: str):
    ...
    if not char_id:
        await msg.finish(I18NContext("zzzerouid.message.card.not_found", name=name))
        return

    data = await zzz_api.get_zzz_enka_data(uid)
    ...
    with open(path / f"{char_id}.json", "w", encoding="utf-8") as f:
        json.dump(data, f, indent=2, ensure_ascii=False)

    await msg.finish(I18NContext("zzzerouid.message.card.refreshed", uid=uid, name=name))


async def draw_card(msg: Bot.MessageSession, name: str):
    uid = await get_uid(msg)
    if not uid:
        await msg.finish(I18NContext("zzzerouid.message.bind_uid_hint"))
        return
    ...
    if not path.exists():
        await msg.finish(I18NContext("zzzerouid.message.card.need_refresh", name=name))
        return
    ...
    for avatar in avatar_list:
        if str(avatar.get("id")) == str(char_id):
            char_data = avatar
            break
    ...
    try:
        img = await _draw_char_detail_image(uid, char_id, char_data)
        await msg.finish(ImageElement.assign(img))
    except Exception:
        from ..utils.logger import logger

        logger.exception("Failed to draw char detail image")
        await _fallback_text(msg, uid, name, char_data)


async def _draw_char_detail_image(
    uid: str,
    char_id: str,
    data: dict,
) -> Image.Image:
    ...
    info_bg = Image.open(TEXT_PATH / "title.png")
    ...
    # 技能部分
    from ..utils.image import get_skill_dict
    ...
    equip_all_bg = Image.open(TEXT_PATH / "prop_bg.png")
...
def char_id_to_sprite(char_id: str) -> str:
    from ..utils.name_convert import char_id_to_sprite as _sprite

    return _sprite(char_id)


async def _fallback_text(msg: Bot.MessageSession, uid: str, name: str, data: dict):
    ...
    lines = [
        ...
        "",
        "Weapon:",
        f"  {weapon.get('name', 'None')} Lv.{weapon.get('level', 0)} R{weapon.get('break_level', 0)}",
        "",
        "Skills:",
    ]
    for skill in data.get("skills", []):
        lines.append(f"  {skill.get('skill_type', '?')}: Lv.{skill.get('level', 0)}")
